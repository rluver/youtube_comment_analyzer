...
# load
source(str_c(getwd(), '/dictionary.R'), encoding = "utf-8")
source(str_c(getwd(), '/function.R'), encoding = "utf-8")
                    ...
                    mainPanel(
                        tabsetPanel(
                            ...
                            tabPanel("Chart", value = 2, tabsetPanel(id = "Chart_View",
                                                                     tabPanel("Chart", value = 2.1, plotlyOutput("out_Chart")),
                            ...
                            tabPanel("Trend", value = 4, tabsetPanel(id = "Trend_View",
                                                                     tabPanel("Trend", value = 4.1, plotlyOutput("out_Trend")),
...
server <- function(input, output, session) {
    ...
    observeEvent(input$in_url, {
        ...
        if(is.null(value$comment) == F){
            value$comment_extracted = withProgress(message = str_c(c("댓글을 분석 중입니다", "잠시만 기다려 주세요"), sep = "\n"), {
                cbind(data.table::data.table(get_comment_extracted(value$comment), str_sub(value$comment$PublishedAt, 1, 10)) %>% 
                          rename(Comment = V1, PublishedAt = V2))
            })
            
            
            updateDateRangeInput(session, 
                                 inputId = "in_date_range",
                                 start = ymd(str_sub(min(value$comment$PublishedAt), 1, 10)),
                                 min = ymd(str_sub(min(value$comment$PublishedAt), 1, 10)),
                                 end = ymd(str_sub(max(value$comment$PublishedAt), 1, 10)),
                                 max = ymd(str_sub(max(value$comment$PublishedAt), 1, 10)))
    ...
    observeEvent(input$in_date_range, {
        ...
        if(is.null(value$comment) == F){
            ...
            output$out_Comment = renderDataTable({
                datatable((value$comment %>% filter(between(ymd(str_sub(PublishedAt, 1, 10)), 
                                                            ymd(input$in_date_range[[1]]),
                                                            ymd(input$in_date_range[[2]]))) %>% 
                               select("AuthorDisplayName", "Comment", "LikeCount")), options = list(
                                   initComplete = JS(
    ...
    observeEvent({
        ...
        input$in_trend_accumulation}, {
            ...
            if(input$in_trend_accumulation == T){
                output$out_Trend = renderPlotly({
                    ggplotly(
                        ggplot(get_ts_info(value$comment_extracted,
                                           value$comment_extracted$Comment,
                                           value$comment_extracted$PublishedAt,
                                           min(value$comment_extracted$PublishedAt),
                                           max(value$comment_extracted$PublishedAt),
                                           input$in_trend_max_word,
                                           input$in_filter_from,
                                           input$in_filter_to,
                                           input$in_delete_word),
    ...
    observeEvent({
        ...
        input$in_apriori_hierarchical}, {
            ...
            # chart
            output$out_Chart = renderPlotly({
                get_comment_dataframe(value$comment_extracted$Comment, 
                                      value$comment_extracted$PublishedAt,
                                      ymd(input$in_date_range[[1]]),
                                      ymd(input$in_date_range[[2]]),
                                      input$in_chart_max_word,
